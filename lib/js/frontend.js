jQuery(document).ready(function(){
	...
	// IE Support for Form Input form attribute
	jQuery( document ).on( 'click', '#apply_coupon', function() {
		jQuery( '#coupon_code' ).val( jQuery( '#coupon_code_frontend' ).val() );
        jQuery( '#coupon_code' ).closest('form').submit();
	});
	// fix coupon remove link in checkout
    jQuery( document ).on( 'click', '.woocommerce-remove-coupon', function(e) {
        jQuery( document ).ajaxComplete(function(){
            window.location = window.location;
        });
    });
    // deactivate keypress enter for woocommerce pages
    jQuery(document).on( 'keypress', '.woocommerce-page', function (e) {
	...
	jQuery( document ).on( 'click', '#send_invite', function() {
		jQuery( '#accept' ).val( jQuery( '#accept_frontend' ).val() );
		jQuery( '#participant_mail' ).val( jQuery( '#participant_mail_frontend' ).val() );
	});
});
